...
RSpec.describe Do::Systemd do
  ...
  describe '#unit_dir' do
    it 'defaults to ~/.config/systemd/user' do
      old = ENV.fetch('XDG_CONFIG_HOME', nil)
      ENV['XDG_CONFIG_HOME'] = '/custom'
      expect(described_class.new.unit_dir).to eq('/custom/systemd/user')
    ...
    it 'falls back to HOME/.config when XDG_CONFIG_HOME is unset' do
      ...
      ENV['HOME'] = '/home/x'
      expect(described_class.new.unit_dir).to eq('/home/x/.config/systemd/user')
      ENV['HOME'] = old_home
  ...
  describe '#show_prop' do
    it 'parses a Property=value line' do
      sd, = make_fake(rules: { 'show' => ['NextElapseUSecRealtime=Mon 2026-08-11 02:00:00 IST', '',
                                          0] })
  ...
  describe '#logs' do
    it 'passes --user -u <unit> and -f when following' do
      ...
      sd = described_class.new(journalctl_bin: jbin)
