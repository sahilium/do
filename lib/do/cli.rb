...
module Do
  ...
  class ConfigCommand < Thor
    ...
    def show
      ...
      config.tasks.each do |task|
        ...
        unless (task.environment || {}).empty?
          say "    environment:      #{task.environment.inspect}"
        end
      end
    end
  end
  ...
  class CLI < Thor
    ...
    def self.is_thor_reserved_word?(word, type)
      false
    end
    ...
    def list
      ...
      say header = format("%-10s  %-9s  %-18s  %s", "TASK", "STATUS", "SCHEDULE", "NEXT")
      say "-" * header.length
      config.tasks.each do |task|
    ...
    def schedule(task = nil)
      config = load_config!(validate: true)
      task = require_task!(task, config)
      raise Do::Error, "task '#{task.name}' has no schedule in the configuration." unless task.scheduled?
      Manager.new(systemd: systemd, config: config).reload
    ...
    def load_config!(validate: true)
      path = config_path
      raise Do::Error, "Configuration file does not exist: #{path}\nCreate it or run 'do config path'." unless File.exist?(path)
      config = Config.load(path)
    ...
    def change_enabled(task_or_name, value)
      ...
      raise Do::Error, "manual-only task '#{task.name}' has no timer to #{value ? "enable" : "disable"}." unless task.scheduled?
    ...
    def run_doctor
      ...
      if systemctl_ok
        check("systemd user manager", systemd.user_available?, nil)
      end
    ...
    def check_generated_units(config)
      ...
      check("generated units (no stale)", stale.empty?, stale.empty? ? nil : "stale: #{stale.join(", ")}")
    end
