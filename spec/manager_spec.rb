...
RSpec.describe Do::Manager do
  ...
  it 'writes service and timer units for scheduled tasks' do
    ...
    bin, = fake_systemctl
    manager = described_class.new(systemd: Do::Systemd.new(systemctl_bin: bin),
    ...
    expect(File.read(File.join(unit_dir,
                               'do-backup.timer'))).to include('OnCalendar=*-*-* 02:00:00')
  end
  ...
  it "unschedule removes the task's units" do
    ...
    manager = described_class.new(systemd: sd, config: config_for(path))
    manager.reload
    unit_dir = File.join(@tmpdir, 'systemd', 'user')
